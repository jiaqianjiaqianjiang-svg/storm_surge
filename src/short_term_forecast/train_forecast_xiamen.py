...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", choices=MODEL_CHOICES, default="cnn", help="短时预报模型结构")
...
def main() -> None:
    ...
    run_name = (
        ...
        f"_{args.frequency}_t{args.input_steps}_h{args.horizon}_{args.model}"
    )
    ...
    plot_validation_timeseries(val_dates, val_obs, val_pred, output_dir / "pred_vs_obs.png")
    ...
    print(f"[OUTPUT] best model: {best_path}")
    print(f"[OUTPUT] loss 曲线: {output_dir / 'loss_curve.png'}")
    print(f"[OUTPUT] 验证集 observed vs predicted 图: {output_dir / 'pred_vs_obs.png'}")
