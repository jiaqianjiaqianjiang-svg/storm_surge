...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-path", type=Path, default=None, help="训练得到的 best_model.pth；不提供时自动按参数搜索")
    parser.add_argument("--data-source", choices=["ERA5"], default=None)
    parser.add_argument("--frequency", choices=["hourly"], default=None)
    parser.add_argument("--start-date", type=str, default="1985-11-01 00:00", help="第一个预报目标时间，例如 1985-12-01 00:00")
    parser.add_argument("--forecast-steps", "--steps", dest="forecast_steps", type=int, default=12, help="向后滚动预报步数")
...
def resolve_checkpoint_arg(args: argparse.Namespace, checkpoint_args: dict[str, object], name: str, default: object) -> object:
    """checkpoint 参数优先；命令行不一致时给出提示。"""

    cli_value = getattr(args, name)
    ckpt_value = checkpoint_args.get(name)
    if ckpt_value is not None:
        if cli_value is not None and cli_value != ckpt_value:
            print(f"[WARN] 命令行 --{name.replace('_', '-')}={cli_value} 与 checkpoint args 中的 {ckpt_value} 不一致，使用 checkpoint 值")
...
def find_default_checkpoint(args: argparse.Namespace) -> Path:
    """没有显式传入 --model-path 时，根据模型名和窗口长度自动寻找 checkpoint。"""

    input_steps = args.input_steps or config.FORECAST_INPUT_STEPS
    model_part = args.model or "*"
    pattern = f"*t{input_steps}_h1_{model_part}/best_model.pth"
    matches = sorted(config.FORECAST_OUTPUT_ROOT.glob(pattern))
    if not matches:
        raise FileNotFoundError(
            f"没有找到默认 checkpoint，搜索目录 {config.FORECAST_OUTPUT_ROOT}，模式 {pattern}。"
            "请显式提供 --model-path。"
        )
    path = matches[-1]
    print(f"[ROLLING] 自动使用 checkpoint: {path}")
    return path
...
def main() -> None:
    ...
    model_path = args.model_path or find_default_checkpoint(args)
    checkpoint = torch.load(model_path, map_location=device)
    scalers = checkpoint["scalers"]
    variables = normalize_variable_names(list(checkpoint.get("variables", config.VARIABLES)))
    grid_size = int(checkpoint.get("grid_size", config.GRID_SIZE))
    ...
    checkpoint_model = checkpoint.get("model_name") or checkpoint_args.get("model")
    if checkpoint_model is not None and args.model is not None and args.model != checkpoint_model:
        print(f"[WARN] 命令行 --model={args.model} 与 checkpoint 模型 {checkpoint_model} 不一致，使用 checkpoint 模型")
    if checkpoint_model is not None:
        args.model = str(checkpoint_model)
    if args.model is None:
        args.model = "cnn"
    if args.data_source != "ERA5" or args.frequency != "hourly":
        raise ValueError("短时预报滚动模块固定使用 ERA5 hourly 模型")
    model = build_model(args.model, input_steps=args.input_steps, n_variables=len(variables), grid_size=grid_size).to(device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
    print(f"[ROLLING] 使用模型: {args.model}")
    ...
    run_name = f"{args.data_source}_{args.frequency}_{args.model}_rolling_{first_target.strftime('%Y%m%d%H')}_n{args.forecast_steps}"
