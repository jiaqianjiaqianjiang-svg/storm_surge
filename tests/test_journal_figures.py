...
def _write_window_run(root: Path, name: str, input_steps: int) -> Path:
    run = root / name
    ...
    _write_prediction_csv(run / "val_predictions.csv", n=36, meters=True)
    return run
...
def _assert_triplet(paths: dict[str, Path] | None) -> None:
    assert paths is not None
    for extension in ("png", "pdf", "svg"):
        assert extension in paths
        assert paths[extension].exists()
        assert paths[extension].stat().st_size > 0


def test_scan_results_and_unit_conversion(tmp_path: Path) -> None:
    _write_window_run(tmp_path, "ERA5_1985_1985_t8_h1", 8)
...
def test_all_individual_plotters_generate_formats(tmp_path: Path) -> None:
    ...
    _write_window_run(results_root, "ERA5_1985_1985_t8_h1", 8)
    ...
    _assert_triplet(plot_window_metrics(experiments, figures))
    _assert_triplet(plot_window_timeseries(experiments, figures))
    _assert_triplet(plot_window_scatter(experiments, figures))

    rolling_csv = rolling_a / "rolling_forecast.csv"
    _assert_triplet(plot_rolling_forecast(rolling_csv, figures))
    _assert_triplet(plot_peak_analysis(rolling_csv, figures))
    _assert_triplet(plot_residual_diagnostics(rolling_csv, figures))
    _assert_triplet(plot_event_comparison([rolling_a / "rolling_forecast.csv", rolling_b / "rolling_forecast.csv"], figures))
...
def test_make_all_journal_figures_manifest(tmp_path: Path) -> None:
    ...
    assert (figures / "figure_event_comparison.pdf").exists()
